...
def weighted_mpjpe(predicted, target, vis, pad):
    assert predicted.shape == target.shape
    l = torch.norm(predicted - target, dim=len(target.shape)-1)
    #vis = torch.squeeze(vis, dim=-2)
    vis = vis[:,pad:pad + 1, :, :].permute(0,1,2,4,3).contiguous()
    vis = torch.squeeze(vis, dim=-1)
    # print("l.shape : "+str(l.shape))
    # print("vis.shape : "+str(vis.shape))
    # input()
    return torch.mean(vis*l)

def weighted_regularized_mpjpe(predicted, target, vis, pad):
    assert predicted.shape == target.shape
    l = torch.norm(predicted - target, dim=len(target.shape) - 1)
    vis = torch.squeeze(vis, dim=-2)
    # vis = torch.mean(vis, dim=1)
    vis = vis[:, pad:pad + 1, :, :].permute(0, 1, 3, 2).contiguous()
    # print("vis.shape : "+str(vis.shape))
    # print("l.shape : " + str(l.shape))
    loss_vis = torch.mean(vis * l, dim=2)
    # print("loss_vis.shape : " + str(loss_vis.shape))
    loss_no_vis = torch.mean(l, dim=2).detach()
    # print("loss_no_vis.shape : " + str(loss_no_vis.shape))

    # print("loss_no_vis regul : " + str(loss_no_vis[0]))
    ratio = torch.where(loss_vis != 0, loss_no_vis / loss_vis.detach(), loss_no_vis)
    # print("loss_vis before regul : " + str(loss_vis[0]))
    loss_vis = loss_vis * ratio
    # print("loss_vis after regul : "+str(loss_vis[0]))
    # input()

    # print("l.shape : "+str(l.shape))
    # print("vis.shape : "+str(vis.shape))
    # input()
    return torch.mean(loss_vis)
