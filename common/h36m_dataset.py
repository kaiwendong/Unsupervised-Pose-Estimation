...
def triangulation(pts, pmat):
    ...
    if len(pmat.shape) == 3:
        ...
        pmat = pmat.to(dev).view(batch, 1, nview, 3, 4).repeat(1, njoint, 1, 1, 1).view(batch * njoint, nview, 3,
                                                                                        4)  # (batch * njoints, nview, 3, 4)
...
def visulization(s, p3d, order=[0, 2, 1], name=''):
    ...
    ax.plot3D(p3d[s, 0, [0, 7, 8, 9, 10], order[0]].cpu(), p3d[s, 0, [0, 7, 8, 9, 10], order[1]].cpu(),
              p3d[s, 0, [0, 7, 8, 9, 10], order[2]].cpu(), 'red')
    ax.plot3D(p3d[s, 0, [8, 14, 15, 16], order[0]].cpu(), p3d[s, 0, [8, 14, 15, 16], order[1]].cpu(),
              p3d[s, 0, [8, 14, 15, 16], order[2]].cpu(), 'red')
    ax.plot3D(p3d[s, 0, [8, 11, 12, 13], order[0]].cpu(), p3d[s, 0, [8, 11, 12, 13], order[1]].cpu(),
              p3d[s, 0, [8, 11, 12, 13], order[2]].cpu(), 'red')
    ax.plot3D(p3d[s, 0, [0, 1, 2, 3], order[0]].cpu(), p3d[s, 0, [0, 1, 2, 3], order[1]].cpu(),
              p3d[s, 0, [0, 1, 2, 3], order[2]].cpu(), 'red')
    ax.plot3D(p3d[s, 0, [0, 4, 5, 6], order[0]].cpu(), p3d[s, 0, [0, 4, 5, 6], order[1]].cpu(),
              p3d[s, 0, [0, 4, 5, 6], order[2]].cpu(), 'red')
...
class Human36mCamera(MocapDataset):
    ...
    def p2d_cam3d_batch(self, p2d, subject_list, view_list, debug=False, extri=None, proj=None,
                        is_predicted_params=True):
        '''
        p2d: (B, T,J, C, N)
        '''
        B, T, J, C, N = p2d.shape
        ...
        trj_w3d = triangulation_torch(p2d.view(-1, J, C, N).permute(0, 1, 3, 2).contiguous(),
                                      prj_mat[:, view_list, ...])  # (B*T, J, 3)
        ...
        if not with_distor:
            ...
            if len(flip_idx) != 0:
                ...
                if gt_2d is not None:
                    ...
                    print("Fliped loss is {}; Unfliped loss is {}".format(
                        mpjpe(p2d_flip.permute(0, 2, 3, 4, 1), gt_2d_flip),
                        mpjpe(p_2d_unflip.permute(0, 2, 3, 4, 1), gt_2d_unflip)))
    ...
    def recover_extri_extri_inv_predicted_params(self, params, subjects):
        """
            ...
            for action, act_data in sub_data.items():
                ...
                for num_view in list(range(2, N_cam + 1)):
                    for view_list in itertools.combinations(list(range(N_cam)), num_view):
                        trj_w3d = triangulation(temp_pred_2d[:, view_list].view(-1, 17, len(view_list), 2),
                                                prj_mat[view_list, ...])
    ...
    def __init__(self, keypoints):
        ...
        for subject, sub_data in keypoints.items():
            ...
            for action, act_data in sub_data.items():
                ...
                cam_norm_3d_trj_homo = torch.cat((cam_norm_3d_trj, torch.ones(cam_norm_3d_trj.shape[0], N_cam, 17, 1)),
                                                 dim=-1)
