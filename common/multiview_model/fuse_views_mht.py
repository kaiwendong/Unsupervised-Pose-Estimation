...
class Pose3dShrink_out(Pose3dShrinkOther):
    ...
    def forward(self, x):
        ...
        x = x.view(B * N, T, -1)
        if self.cfg.TRAIN.TEMPORAL_SMOOTH_LOSS_WEIGHT is not None:
...
class fuse_views_mht(nn.Module):
    def __init__(self, cfg, dropout=0.1, momentum=0.1, is_train=False, num_joints=17, ):
        ...
        # proposed mhf
        self.mhf = MHF(cfg, in_channels=self.in_channels, emb_size=self.hidden, T=self.T, dropout=dropout,
                       num_joints=num_joints, istrain=is_train)
