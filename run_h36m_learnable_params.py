...
if True:
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action, batch_flip in train_generator.next_epoch():
            ...
            extri, extri_inv = HumanCam.recover_extri_extri_inv(model.module.learn_theta, model.module.learn_t)
            ...
            out_world = HumanCam.p3dcam_3dwd_batch(out, sub_action, view_list[:4], extri_inv=extri_inv)

            out_world = torch.squeeze(torch.mean(out_world, dim=1), dim=1)
            # print(out.shape)
            # input()

            pose_3D_in_cam_space = HumanCam.p3dwd_p3dcam_batch(out_world, sub_action, view_list, extri=extri)
