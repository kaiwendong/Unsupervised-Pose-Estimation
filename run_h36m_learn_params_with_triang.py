...
if True:
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action, batch_flip in train_generator.next_epoch():
            ...
            prj_2dpre_to_3d = HumanCam.p2d_cam3d_batch(h36_inp[:, pad:pad + 1, :, :, :4].to(out.device), sub_action, view_list[:4],
            ...
            pose_2D_from3D = HumanCam.p3d_im2d_batch(prj_2dpre_to_3d, sub_action, view_list, with_distor=True, flip=batch_flip)
            ...
            loss_copy = mpjpe(prj_2dpre_to_3d.permute(0,1,2,4,3).contiguous(), p3d_gt_abs[:, pad:pad + 1].permute(0,1,2,4,3).contiguous())
            if summary_writer is not None and cfg.TRAIN.UNSUPERVISE == True:
                        ...
                        for valid_subject in cfg.H36M_DATA.SUBJECTS_TEST:
                            for act in vis_actions if EVAL else actions:
                                ...
                                for batch_2d, _, _ in test_generator.next_epoch():
                                    ...
                                    prj_2dpre_to_3d = HumanCam.p2d_cam3d_batch(h36_inp[:, pad:pad + 1, :, :, :4], sub_action, view_list[:4], debug=False, extri=extri, proj=proj)
