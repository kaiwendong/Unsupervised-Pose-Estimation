...
if True:
    ...
    train_generator = ChunkedGenerator(cfg.TRAIN.BATCH_SIZE, poses_train_2d, 1, pad=pad, causal_shift=causal_shift,
                                       ...
                                       extra_poses_3d=poses_3d_extra) if cfg.TRAIN.LOAD_SUB == True else ChunkedGenerator(
        cfg.TRAIN.BATCH_SIZE, poses_train_2d, 1, pad=pad, causal_shift=causal_shift, shuffle=True,
        augment=cfg.TRAIN.FLIP_AUG, kps_left=kps_left, kps_right=kps_right, joints_left=joints_left,
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action, batch_flip in train_generator.next_epoch():
            ...
            loss_consis_weight = cfg.TRAIN.CONSIS_LOSS_WEIGHT if cfg.TRAIN.VISI_WEIGHT == False else vis[:, pad:pad + 1,
                                                                                                     :, :, :4]
            loss_consis = 0
            # ********************************************************************************************
            ...
            if cfg.TRAIN.TEMPORAL_SMOOTH_LOSS_WEIGHT is not None:
                ...
                smooth_err = (smooth_err_2d - smooth_target) if cfg.TRAIN.ERR_BTW_FULL_MID == False else (
                                                                                                                     smooth_err_2d - smooth_target) + err_bwt_full_mid
            ...
            loss_total = loss
            print('Final Loss is {}'.format(loss_total))
            if cfg.TRAIN.USE_INTER_LOSS:
                                    ...
                                    if cfg.TEST.TEST_FLIP:
                                        if cfg.NETWORK.USE_GT_TRANSFORM:
                                            rotation = get_rotation(
                                                torch.cat((inputs_3d_gt, inputs_3d_gt_flip), dim=0)[..., view_list])
                                            rotation = rotation.repeat(1, 1, 1, inp.shape[1], 1, 1)
                                        else:
                                            rotation = None
                                        print('Evaluation part! Input size={}, input_fip={}'.format(inp.shape,
                                                                                                    inp_flip.shape))
