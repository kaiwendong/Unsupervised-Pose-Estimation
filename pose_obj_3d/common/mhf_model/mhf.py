...
class MHF(nn.Module):
    def __init__(self, cfg, in_channels, emb_size, T=7, dropout=0.1, num_joints=17, momentum = 0.1, istrain = False):
        ...
        self.max_view_num = len(cfg.OBJ_POSE_DATA.TRAIN_CAMERAS) + cfg.TRAIN.NUM_AUGMENT_VIEWS
        if istrain:
            self.view_num = len(cfg.OBJ_POSE_DATA.TRAIN_CAMERAS) + cfg.TRAIN.NUM_AUGMENT_VIEWS
        else:
            self.view_num = len(cfg.OBJ_POSE_DATA.TRAIN_CAMERAS)
        ...
        self.view_embedding = nn.Parameter(torch.zeros(1, 1, self.view_num, 1)).repeat(self.cfg.TRAIN.BATCH_SIZE*(self.max_view_num), T, 1, self.hidden)
        # intermidiate shrink
        self.inter_shrink = nn.Linear(self.hidden*len(cfg.OBJ_POSE_DATA.TRAIN_CAMERAS), self.hidden*3)
