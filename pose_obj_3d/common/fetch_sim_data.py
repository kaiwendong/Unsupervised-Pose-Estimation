...
def fetch_simdata_frm_scrw(path, sub_list, debug):
    keypoints = {}
    datapath = '../data/RLBench_pose_est_data/keypoints_from_3D_full_pose_with_uncertainty_3X_noise.npz'
    if debug:
        keypoints_or = {}
    ...
    for tk_exp in keypoint['S1'].item().keys():
        ...
        print('Processing {}'.format(tk_exp))
...
def main():
    train_actions = []
    r_src = fetch_simdata_frm_scrw('../data/RLBench_pose_est_data', [0,8,41,0,84], debug=False)
    np.savez('sim_all_formal_all_task_with_uncertainty_3X_noise.npz', pose=r_src)
