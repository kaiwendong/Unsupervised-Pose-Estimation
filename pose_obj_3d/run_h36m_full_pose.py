...
keypoints = dict(np.load(cfg.OBJ_POSE_DATA.ROOT_DIR+'/sim_all_formal_600.npz', allow_pickle=True))['pose'].item()
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action in train_generator.next_epoch():
            ...
            if cfg.NETWORK.USE_GT_TRANSFORM or cfg.TRAIN.USE_ROT_LOSS:
                #相机之间的旋转
                #print(pos_gt.shape)
                center_point_3D_gt = torch.unsqueeze(pos_gt[:, :, :, :3, :], 3).repeat((1, 1, 1, 8, 1, 1))

                #print("center_point_3D_gt.shape : "+str(center_point_3D_gt.shape))
                """pos_gt[:, :, :, 3, :]=1
                pos_gt[:, :, :, 4, :] = 2
                pos_gt[:, :, :, 5, :] = 3"""
                dims_point_3D_gt = torch.unsqueeze(pos_gt[:, :, :, 3:6, :], 4).repeat((1, 1, 1, 1, 2, 1))
                #print("dims_point_3D_gt.shape : " + str(dims_point_3D_gt.shape))
                sh_ = dims_point_3D_gt.shape
                dims_point_3D_gt = torch.reshape(dims_point_3D_gt, sh_[:3]+(sh_[3]*sh_[4], sh_[5]))
                dims_point_3D_gt[:, :, :, [0, 2, 4], :]=-1*dims_point_3D_gt[:, :, :, [0, 2, 4], :]
                offsets = dims_point_3D_gt/2
                offsets = offsets[:, :, :, point_indexes, :]
                #print(offsets[0, 0, 0, :, :, 0])
                #print("offsets.shape : " + str(offsets.shape))
                #rotate offsets here
                sh_ = offsets.shape
                #offsets = torch.reshape(offsets, sh_[:3]+(sh_[3]*sh_[4], sh_[5])).permute((0,1,2,4,3)).contiguous()
                offsets = offsets.permute((0, 1, 2, 3, 5, 4)).contiguous()
                quat_point_3D_gt = torch.unsqueeze(pos_gt[:, :, :, 6:, :], 3).repeat((1, 1, 1, 8, 1, 1)).permute((0, 1, 2, 3, 5, 4)).contiguous()
                #print("quat_point_3D_gt.shape : "+str(quat_point_3D_gt.shape))
                offsets = qv_mult_batch(quat_point_3D_gt, offsets)
                #print("offsets.shape : " + str(offsets.shape))
                offsets = offsets[..., :3].permute((0, 1, 2, 3, 5, 4)).contiguous()
                #print("offsets.shape : " + str(offsets.shape))
                point_3D_gt = center_point_3D_gt + offsets
                point_3D_gt = point_3D_gt.permute((0,1,5,2,3,4)).contiguous()

                #obj_center_cam_space = pos_gt[:, pad:pad + 1, :, :, :3]
                #print("point_3D_gt.shape : " + str(point_3D_gt.shape))
                init_shape = point_3D_gt.shape
                point_3D_gt = torch.cat((point_3D_gt, torch.ones(point_3D_gt.shape[:-1] + (1,)).cuda()), axis=-1)
                #print("addined ones : point_3D_gt.shape : " + str(point_3D_gt.shape))
                point_3D_gt = torch.permute(point_3D_gt.view(*point_3D_gt.shape[:3], -1, 4),(0, 1, 3, 2, 4)).contiguous()
                #print("permuted : point_3D_gt.shape : " + str(point_3D_gt.shape))
                point_3D_gt = torch.einsum('voi, btpvi->btpvo', extrinsics, point_3D_gt)
                #print("extr multiplied : point_3D_gt.shape : " + str(point_3D_gt.shape))
                point_3D_gt = torch.permute(point_3D_gt, (0, 1, 2, 4, 3)).contiguous()[:, :, :,  :3, :]
                #np.save("points_3D.npy", point_3D_gt[0, 0].detach().cpu().numpy())
                #np.save("points_2D.npy", inputs_2d_gt[0, 0].detach().cpu().numpy())
                #input()

                """print(offsets[0, 0, 0, 0, :, 0])
                x = point_3D_gt[0, 0, :, :, 0, 0].detach().cpu().numpy()
                y = point_3D_gt[0, 0, :, :, 1, 0].detach().cpu().numpy()
                z = point_3D_gt[0, 0, :, :, 2, 0].detach().cpu().numpy()"""
                ...
                #sh_ = point_3D_gt.shape
                #point_3D_gt = torch.reshape(point_3D_gt, sh_[:2]+(sh_[2]*sh_[3], sh_[4], sh_[5]))
                rotation = get_rotation(point_3D_gt[:, :1])
                # #相机之间的位移
                # #print(rotation)
            ...
            for x_i in [0, 1]:
                for y_i in [2, 3]:
                    for z_i in [4, 5]:
                        ...
                        print(pt)"""
            """print(pos_gt.shape)
            print(pos_gt[0, 0, 3, 3, :])
            print(inp[0, 0, 3, :, 3])"""
            #input()
            #points_3d_world = pos_gt[:,pad:pad+1][:, :, :, :, ]
            #print("pos_gt.shape : "+str(pos_gt.shape))
            obj_center_cam_space = pos_gt[:,pad:pad+1, :, :, :3]
            #print("obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
            init_shape = obj_center_cam_space.shape
            obj_center_cam_space = torch.cat((obj_center_cam_space, torch.ones(obj_center_cam_space.shape[:-1] + (1,)).cuda()), axis=-1)
            #print("addined ones : obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
            obj_center_cam_space = torch.permute(obj_center_cam_space.view(*obj_center_cam_space.shape[:3], -1, 4), (0, 1, 3, 2, 4)).contiguous()
            #print("permuted : obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
            obj_center_cam_space = torch.einsum('voi, btpvi->btpvo', extrinsics, obj_center_cam_space)
            #print("extr multiplied : obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
            obj_center_cam_space = torch.permute(obj_center_cam_space, (0, 1, 3, 2, 4)).contiguous()[:, :, :, :, :3]
            #print("permuted back : obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
            ...
            """np.save("mean_obj_center_cam_space_720_ex.npy",(np.mean(obj_center_cam_space.detach().cpu().numpy(), axis=(0, 1), keepdims=True)))
            np.save("std_obj_center_cam_space_720_ex.npy",np.std(obj_center_cam_space.detach().cpu().numpy(), axis=(0, 1), keepdims=True))
            np.save("min_obj_center_cam_space_720_ex.npy",np.min(obj_center_cam_space.detach().cpu().numpy(), axis=(0, 1), keepdims=True))
            np.save("max_obj_center_cam_space_720_ex.npy",np.max(obj_center_cam_space.detach().cpu().numpy(), axis=(0, 1), keepdims=True))"""
            ...
            # input()
            """print("basic loss : ")
            print("pos_gt[:, 3:4].shape : " + str(pos_gt[:, 3:4].shape))
            print("min_full_pose_720_ex.shape : " + str(min_full_pose_720_ex.shape))"""
            normed_gt_dims = torch.where(range_full_pose_720_ex[..., 3:6] != 0, (pos_gt[:, 3:4, :, :, 3:6]-min_full_pose_720_ex[..., 3:6])/range_full_pose_720_ex[..., 3:6], 0)
            normed_gt_quats = torch.where(range_full_pose_720_ex[..., 6:] != 0, (pos_gt[:, 3:4, :, :, 6:]-min_full_pose_720_ex[..., 6:])/range_full_pose_720_ex[..., 6:], 0)

            normed_obj_center_cam_space = torch.where(range_obj_center_cam_space_720_ex != 0, (obj_center_cam_space-min_obj_center_cam_space_720_ex)/range_obj_center_cam_space_720_ex, 0)

            #normed_gt[..., :3] = normed_obj_center_cam_space

            #print("out.shape : "+str(out.shape))
            #print("normed_gt.shape : " + str(normed_gt.shape))
            loss_dims = mpjpe(out[...,3:6], normed_gt_dims)
            loss_quat = mpjpe(out[...,6:], normed_gt_quats)
            loss_center = mpjpe(out[...,:3], normed_obj_center_cam_space)

            loss = loss_dims + loss_quat + loss_center
            ...
            plt.show()"""
            un_normed_pred_center = out[...,:3]*range_obj_center_cam_space_720_ex+min_obj_center_cam_space_720_ex
            un_normed_dims = out[...,3:6] * range_full_pose_720_ex[...,3:6] + min_full_pose_720_ex[...,3:6]
            un_normed_quat = out[...,6:] * range_full_pose_720_ex[...,6:] + min_full_pose_720_ex[...,6:]


            # print("un_normed_pred_center[:, :, :, :, :3].shape : "+str(un_normed_pred_center[:, :, :, :, :3].shape))
            # print("obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
            #
            # print("un_normed_pred_center[:, :, :, :, 3:6].shape : " + str(un_normed_pred_center[:, :, :, :, 3:6].shape))
            # print("pos_gt[:, 3:4, :, :, 3:6].shape : " + str(pos_gt[:, 3:4, :, :, 3:6].shape))

            """print("center gt : "+str(normed_obj_center_cam_space[:6, 0, 0, :, :]))
            print("dims gt : " + str(normed_gt_dims[:6, 0, 0, :, :]))
            print("quat gt : " + str(normed_gt_quats[:6, 0, 0, :, :]))

            print("center gt : " + str(out[:6, 0, 0, :, :3]))
            print("dims gt : " + str(out[:6, 0, 0, :, 3:6]))
            print("quat gt : " + str(out[:6, 0, 0, :, 6:]))"""


            loss_center = mpjpe(un_normed_pred_center, obj_center_cam_space)
            loss_dims = mpjpe(un_normed_dims, pos_gt[:, 3:4, :, :, 3:6])
            loss_quat = mpjpe(un_normed_quat, pos_gt[:, 3:4, :, :, 6:])
            loss_unnormed = loss_center + loss_dims + loss_quat
            ...
            if summary_writer is not None:
                ...
                summary_writer.add_scalar("loss_unnormed_center/iter", loss_center, iters)
                summary_writer.add_scalar("loss_unnormed_quat/iter", loss_quat, iters)
                summary_writer.add_scalar("loss_unnormed_dims/iter", loss_dims, iters)
        ...
        with torch.no_grad():
            ...
            eval_results_center = np.zeros((len(cfg.TEST.NUM_FRAMES), len(cfg.OBJ_POSE_DATA.SUBJECTS_TRAIN), len(cfg.TEST.NUM_VIEWS), NUM_VIEW, 4))
            eval_results_dims =  np.zeros((len(cfg.TEST.NUM_FRAMES), len(cfg.OBJ_POSE_DATA.SUBJECTS_TRAIN), len(cfg.TEST.NUM_VIEWS), NUM_VIEW, 4))
            eval_results_quat = np.zeros((len(cfg.TEST.NUM_FRAMES), len(cfg.OBJ_POSE_DATA.SUBJECTS_TRAIN), len(cfg.TEST.NUM_VIEWS), NUM_VIEW, 4))
            t_len_id = 0
            for t_len in cfg.TEST.NUM_FRAMES:
                """eval_results.append([])
                eval_results_center.append([])
                eval_results_dims.append([])
                eval_results_quat.append([])"""
                epoch_loss_valid = 0  
                action_mpjpe = {}
                ...
                for num_view in cfg.TEST.NUM_VIEWS:
                    ...
                    eval_results_center[-1].append([])
                    eval_results_dims[-1].append([])
                    eval_results_quat[-1].append([])"""
                    pad_t = t_len // 2
                    view_list_id=0
                    for view_list in itertools.combinations(list(range(NUM_VIEW)), num_view):
                        """eval_results[-1][-1].append([])
                        eval_results_center[-1][-1].append([])
                        eval_results_dims[-1][-1].append([])
                        eval_results_quat[-1][-1].append([])"""
                        ...
                        for valid_subject in cfg.OBJ_POSE_DATA.SUBJECTS_TEST:
                            ...
                            eval_results_center[-1][-1][-1].append([])
                            eval_results_dims[-1][-1][-1].append([])
                            eval_results_quat[-1][-1][-1].append([])"""
                            n_batch = 0
                            for act in vis_actions if EVAL else test_actions:

                                """eval_results[-1][-1][-1][-1].append([[], [], [], []])
                                eval_results_center[-1][-1][-1][-1].append([[], [], [], []])
                                eval_results_dims[-1][-1][-1][-1].append([[], [], [], []])
                                eval_results_quat[-1][-1][-1][-1].append([[], [], [], []])"""
                                """if n_batch >= 5:
                                    break"""
                                ...
                                for batch_2d, _ in test_generator.next_epoch():
                                    ...
                                    inputs_3d_gt = torch.max(inputs_3d_gt, 4, keepdim=True)[0].repeat(1, 1, 1, 1, 4)
                                    pos_gt = inputs_3d_gt[..., view_list]
                                    obj_center_cam_space = pos_gt[:, :, :, :3, :].permute((0, 1, 4, 2, 3)).contiguous()
                                    print(obj_center_cam_space.shape)
                                    # print("obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
                                    init_shape = obj_center_cam_space.shape
                                    obj_center_cam_space = torch.cat((obj_center_cam_space, torch.ones(obj_center_cam_space.shape[:-1] + (1,)).cuda()), axis=-1)
                                    # print("addined ones : obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
                                    ...
                                    # print("extr multiplied : obj_center_cam_space.shape : " + str(obj_center_cam_space.shape))
                                    obj_center_cam_space = torch.permute(obj_center_cam_space, (0, 1, 2, 4, 3)).contiguous()[:, :, :, :3, :]
                                    print(inputs_3d_gt.shape)
                                    print(obj_center_cam_space.shape)
                                    inputs_3d_gt[:, :, :, :3] = obj_center_cam_space
                                    ...
                                    #print(out.permute((0, 1, 4, 2, 3)).contiguous().shape)
                                    #print("range_full_pose_720_ex.shape : "+str(range_full_pose_720_ex.shape))
                                    #out = out*range_full_pose_720_ex_eval+min_full_pose_720_ex_eval

                                    un_normed_pred_center = out[:,:,:, :3, :] * range_obj_center_cam_space_720_ex_eval + min_obj_center_cam_space_720_ex_eval
                                    un_normed_dims = out[:,:,:, 3:6, :] * range_full_pose_720_ex_eval[:,:,:, 3:6, :] + min_full_pose_720_ex_eval[:,:,:, 3:6, :]
                                    un_normed_quat = out[:,:,:,  6:, :] * range_full_pose_720_ex_eval[:,:,:,  6:, :] + min_full_pose_720_ex_eval[:,:,:,  6:, :]
                                    if EVAL and n_batch==10:
                                        print("un_normed_pred_center.shape : "+str(un_normed_pred_center.shape))
                                        print("inputs_3d_gt.shape : "+str(inputs_3d_gt.shape))
                                        fig = plt.figure("View 0")
                                        x = un_normed_pred_center[1, 0, :, 0, 0].detach().cpu().numpy()
                                        y = un_normed_pred_center[1, 0, :, 1, 0].detach().cpu().numpy()
                                        z = un_normed_pred_center[1, 0, :, 2, 0].detach().cpu().numpy()

                                        ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='o', c='r')

                                        x = inputs_3d_gt[1, 0, :, 0, 0].detach().cpu().numpy()
                                        y = inputs_3d_gt[1, 0, :, 1, 0].detach().cpu().numpy()
                                        z = inputs_3d_gt[1, 0, :, 2, 0].detach().cpu().numpy()
                                        #fig = plt.figure("visu")
                                        #ax = plt.axes(projection='3d')
                                        #ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='^')

                                        fig = plt.figure("View 1")
                                        x = un_normed_pred_center[1, 0, :, 0, 1].detach().cpu().numpy()
                                        y = un_normed_pred_center[1, 0, :, 1, 1].detach().cpu().numpy()
                                        z = un_normed_pred_center[1, 0, :, 2, 1].detach().cpu().numpy()
                                        ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='o', c='r')

                                        x = inputs_3d_gt[1, 0, :, 0, 1].detach().cpu().numpy()
                                        y = inputs_3d_gt[1, 0, :, 1, 1].detach().cpu().numpy()
                                        z = inputs_3d_gt[1, 0, :, 2, 1].detach().cpu().numpy()
                                        # fig = plt.figure("visu")
                                        # ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='^')

                                        fig = plt.figure("View 2")
                                        x = un_normed_pred_center[1, 0, :, 0, 2].detach().cpu().numpy()
                                        y = un_normed_pred_center[1, 0, :, 1, 2].detach().cpu().numpy()
                                        z = un_normed_pred_center[1, 0, :, 2, 2].detach().cpu().numpy()
                                        ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='o', c='r')

                                        x = inputs_3d_gt[1, 0, :, 0, 2].detach().cpu().numpy()
                                        y = inputs_3d_gt[1, 0, :, 1, 2].detach().cpu().numpy()
                                        z = inputs_3d_gt[1, 0, :, 2, 2].detach().cpu().numpy()
                                        # fig = plt.figure("visu")
                                        # ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='^')

                                        fig = plt.figure("View 3")
                                        x = un_normed_pred_center[1, 0, :, 0, 3].detach().cpu().numpy()
                                        y = un_normed_pred_center[1, 0, :, 1, 3].detach().cpu().numpy()
                                        z = un_normed_pred_center[1, 0, :, 2, 3].detach().cpu().numpy()
                                        ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='o', c='r')

                                        x = inputs_3d_gt[1, 0, :, 0, 3].detach().cpu().numpy()
                                        y = inputs_3d_gt[1, 0, :, 1, 3].detach().cpu().numpy()
                                        z = inputs_3d_gt[1, 0, :, 2, 3].detach().cpu().numpy()
                                        # fig = plt.figure("visu")
                                        # ax = plt.axes(projection='3d')
                                        # ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x, y, z, marker='^')
                                    ...
                                    for idx, view_idx in enumerate(view_list):
                                        ...
                                        loss_view_tmp_center = eval_metrc(cfg, un_normed_pred_center[..., idx], inputs_3d_gt[:, :, :, :3, view_idx])
                                        loss_view_tmp_dims = eval_metrc(cfg, un_normed_dims[..., idx], inputs_3d_gt[:, :, :, 3:6,  view_idx])
                                        loss_view_tmp_quat = eval_metrc(cfg, un_normed_quat[..., idx], inputs_3d_gt[:, :, :, 6:,  view_idx])
                                        loss_view_tmp = loss_view_tmp_center + loss_view_tmp_dims + loss_view_tmp_quat
                                        ...
                                        eval_results_center[t_len_id, valid_subject_id, num_view_id, view_list_id, idx] +=loss_view_tmp_center
                                        eval_results_dims[t_len_id, valid_subject_id, num_view_id, view_list_id, idx] += loss_view_tmp_dims
                                        eval_results_quat[t_len_id, valid_subject_id, num_view_id, view_list_id, idx] += loss_view_tmp_quat
                                        """eval_results[-1][-1][-1][-1][-1].append(loss_view_tmp.detach().cpu().numpy())
                                        eval_results_center[-1][-1][-1][-1][-1].append(loss_view_tmp_center.detach().cpu().numpy())
                                        eval_results_dims[-1][-1][-1][-1][-1].append(loss_view_tmp_dims.detach().cpu().numpy())
                                        eval_results_quat[-1][-1][-1][-1][-1].append(loss_view_tmp_quat.detach().cpu().numpy())"""
                            ...
                            eval_results_center[t_len_id, valid_subject_id, num_view_id, view_list_id, :] /= n_batch
                            eval_results_dims[t_len_id, valid_subject_id, num_view_id, view_list_id, :] /= n_batch
                            eval_results_quat[t_len_id, valid_subject_id, num_view_id, view_list_id, :] /= n_batch
                ...
                for task_idx in range(len(cfg.OBJ_POSE_DATA.SUBJECTS_TEST)):
                    ...
                    for i in range(NUM_VIEW):
                        ...
                        summary_writer.add_scalar("test_mpjpe_tsk{}_view{}_center/epoch".format(tsk, i), eval_results_center[0, task_idx, 0, 0, i], epoch)
                        summary_writer.add_scalar("test_mpjpe_tsk{}_view{}_dims/epoch".format(tsk, i), eval_results_dims[0, task_idx, 0, 0, i], epoch)
                        summary_writer.add_scalar("test_mpjpe_tsk{}_view{}_quat/epoch".format(tsk, i), eval_results_quat[0, task_idx, 0, 0, i], epoch)
