...
keypoints = dict(np.load(cfg.OBJ_POSE_DATA.ROOT_DIR+'/sim_all_formal_all_task_with_uncertainty.npz', allow_pickle=True))['pose'].item()
...
def fetch(subjects, action_filter=None, parse_3d_poses=True, is_test=False):
    ...
    out_camera_params = []
    out_subject_action = []
    used_cameras = cfg.OBJ_POSE_DATA.TEST_CAMERAS if is_test else cfg.OBJ_POSE_DATA.TRAIN_CAMERAS
    # print(keypoints.keys())
...
if True:
    ...
    num_train_views = len(cfg.OBJ_POSE_DATA.TRAIN_CAMERAS) + cfg.TRAIN.NUM_AUGMENT_VIEWS
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action in train_generator.next_epoch():
            ...
            #print(pos_gt[0, 0, :, 0, :])
            pos_gt = torch.max(pos_gt, 2, keepdim=True)[0].repeat(1, 1, len(view_list), 1, 1)
            ...
            input()"""
            print(out.shape)
            out = torch.mean(out, 2, keepdim=True).repeat(1, 1, len(view_list), 1, 1)
            ...
            print(out_unsup.shape)
            out_unsup = torch.permute(out_unsup.view(*out_unsup.shape[:3], -1, 4), (0, 1, 3, 2, 4)).contiguous()
            point_im_cam = torch.einsum('voi, btpvi->btpvo', full_transf[view_list], out_unsup)
        ...
        with torch.no_grad():
            if not cfg.TEST.TRIANGULATE:
                ...
                model_test.eval()
            NUM_VIEW = len(cfg.OBJ_POSE_DATA.TEST_CAMERAS)
            if EVAL:
            ...
            for t_len in cfg.TEST.NUM_FRAMES:
                        ...
                        for valid_subject in cfg.OBJ_POSE_DATA.SUBJECTS_TEST:
                            ...
                            for act in vis_actions if EVAL else test_actions:
                                ...
                                for batch_2d, _ in test_generator.next_epoch():
                                    ...
                                    out = out.detach()#.cpu()
                                    #print("out.shape : "+str(out.shape))
                                    out = torch.mean(out, 4, keepdim=True).repeat(1, 1, 1, 1, len(view_list))
                                    ...
                                    if EVAL:
                                        ...
                                        np.save("eval_points/unsup/"+str(valid_subject)+"/out_"+str(n_batch), out_.detach().cpu().numpy())
                                        np.save("eval_points/unsup/"+str(valid_subject)+"/pos_gt_"+str(n_batch), pos_gt_.detach().cpu().numpy())
                                        ...
                                        # print("point_im_cam.shape : "+str(point_im_cam.shape))
                                        cam_idx = 0
                                        np.save("eval_points/unsup/"+str(valid_subject)+"/inputs_2d_gt" + str(n_batch), inputs_2d_gt.detach().cpu().numpy())
                                        np.save("eval_points/unsup/"+str(valid_subject)+"/inputs_2d_pre" + str(n_batch), inputs_2d_pre.detach().cpu().numpy())
                                        np.save("eval_points/unsup/"+str(valid_subject)+"/point_im_cam_" + str(n_batch), point_im_cam.detach().cpu().numpy())
                                        for cam_name in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                                            ...
                                            #np.save("point_im_cam_" + str(n_batch)+"_cam_"+str(cam_name), point_im_cam.detach().cpu().numpy())
                                    ...
                                    for idx, view_idx in enumerate(view_list):
                                        ...
                                        loss_view_tmp = eval_metrc(cfg, out[..., idx][..., :3, :], inputs_3d_gt[:, :, :, :, view_idx][..., :3, :])
                ...
                for num_view in cfg.TEST.NUM_VIEWS:
                    ...
                    for act in action_mpjpe:
                        ...
                        for i in range(NUM_VIEW):
                            print('view_{}: {:.3f}'.format(cfg.OBJ_POSE_DATA.TEST_CAMERAS[i], action_mpjpe[act][num_view - 1][i] * 1000), end = '    ')
