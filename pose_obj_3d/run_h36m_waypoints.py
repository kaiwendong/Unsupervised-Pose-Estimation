                ...
                for num_view in cfg.TEST.NUM_VIEWS:
                    ...
                    for view_list in itertools.combinations(list(range(NUM_VIEW)), num_view):
                        ...
                        for valid_subject in cfg.OBJ_POSE_DATA.SUBJECTS_TEST:
                            eval_results[-1][-1][-1].append([[] for _ in range(len(view_list))])
                            n_batch = 0
                            ...
                            for act in vis_actions if EVAL else test_actions:
                                ...
                                for batch_2d, _ in test_generator.next_epoch():
                                    ...
                                    if EVAL:
                                        ...
                                        np.save("eval_points/sup/"+str(valid_subject)+"/out_"+str(n_batch), out_.detach().cpu().numpy())
                                        np.save("eval_points/sup/"+str(valid_subject)+"/pos_gt_"+str(n_batch), pos_gt_.detach().cpu().numpy())
                                        ...
                                        cam_idx = 0
                                        np.save("eval_points/sup/"+str(valid_subject)+"/inputs_2d_gt" + str(n_batch), inputs_2d_gt.detach().cpu().numpy())
                                        np.save("eval_points/sup/"+str(valid_subject)+"/inputs_2d_pre" + str(n_batch), inputs_2d_pre.detach().cpu().numpy())
                                        np.save("eval_points/sup/"+str(valid_subject)+"/point_im_cam_" + str(n_batch), point_im_cam.detach().cpu().numpy())
