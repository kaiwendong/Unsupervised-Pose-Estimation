...
def fetch(subjects, action_filter=None, parse_3d_poses=True, is_test=False, out_plus=False):
    ...
    out_poses_3d = []
    out_subject_action = []
    used_cameras = [0,1,2,3]#cfg.H36M_DATA.TEST_CAMERAS if is_test else cfg.H36M_DATA.TRAIN_CAMERAS
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action, batch_flip in train_generator.next_epoch():
            ...
            if cfg.TRAIN.NUM_AUGMENT_VIEWS > 0:
                ...
                pos_gt = pos_gt_3d[..., [0, 1, 2, 3]]

            else:
                if use_2d_gt:
                    inp_4_views =inputs_2d_gt
                    h36_inp = inputs_2d_gt[..., view_list]
                else:
                    inp_4_views = inputs_2d_pre
                    h36_inp = inputs_2d_pre[..., view_list]
                pos_gt = inputs_3d_gt[..., [0, 1, 2, 3]]
            ...
            print("h36_inp.shape : " + str(h36_inp.shape))
            ...
            out = out+root_in_world[:, 0:1]
            # print(out.shape)
            # input()
            pose_3D_in_cam_space = HumanCam.p3dwd_p3dcam_batch(out[:, 0, :, :], sub_action, [0,1,2,3])
            ...
            # input()
            pose_2D_from3D = HumanCam.p3d_im2d_batch(pose_3D_in_cam_space, sub_action, [0,1,2,3], with_distor=True,
                                                     flip=batch_flip)
            ...
            inputs_2d_gt = inp_4_views.permute(0, 1, 4, 2, 3).contiguous()
            ...
            print(inputs_2d_gt[:, pad:pad + 1, :, :].shape)
            loss_consis_wd = mpjpe(pose_2D_from3D, inputs_2d_gt[:, pad:pad + 1, :, :].to(out.device))
                        ...
                        for valid_subject in cfg.H36M_DATA.SUBJECTS_TEST:
                            for act in vis_actions if EVAL else actions:
                                ...
                                for batch_2d, _, _ in test_generator.next_epoch():
                                    ...
                                    #out = out + p3d_root
                                    # print(inputs_2d_pre[:,pad_t].shape)
                                    # print(out.shape)
                                    # print(inputs_3d_gt.shape)
                                    # input()
