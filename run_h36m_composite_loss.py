...
if True:
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action, batch_flip in train_generator.next_epoch():
            ...
            prj_2dpre_to_3d = HumanCam.p2d_cam3d_batch(h36_inp[:, pad:pad + 1, :, :, :4], sub_action, view_list[:4],
                                                       debug=False)
            # print(prj_2dpre_to_3d.permute(0, 1, 2, 4, 3).contiguous().shape)
            # print(out.permute(0, 1, 2, 4, 3).contiguous().shape)
            # # input()
            # print(vis[:, pad:pad + 1].permute(0, 1, 2, 4, 3).contiguous().shape)
            # input()

            weight_triang = vis#[:, pad:pad + 1].permute(0, 1, 2, 4, 3).contiguous()
            # print(prj_2dpre_to_3d.permute(0, 1, 2, 4, 3).contiguous().shape)
            # print(weight_triang.shape)
            # input()
            loss_3D_triang = weighted_mpjpe(prj_2dpre_to_3d.permute(0, 1, 2, 4, 3).contiguous().to(out.device), out.permute(0, 1, 2, 4, 3).contiguous(), weight_triang.to(out.device), pad)

            out = out + p3d_root[:, pad:pad + 1]
            out_world = HumanCam.p3dcam_3dwd_batch(out, sub_action, view_list[:4])

            out_world = torch.squeeze(torch.mean(out_world, dim=1), dim=1)
            # print(out.shape)
            # input()

            pose_3D_in_cam_space = HumanCam.p3dwd_p3dcam_batch(out_world, sub_action, view_list)
            ...
            # input()
            pose_2D_from3D = HumanCam.p3d_im2d_batch(pose_3D_in_cam_space, sub_action, view_list, with_distor=True,
                                                     flip=batch_flip)
            ...
            # input()
            weight_2d = 1.085-vis
            # print(pose_2D_from3D.shape)
            # print(weight_2d.shape)
            # input()
            loss_consis_wd = weighted_mpjpe(pose_2D_from3D.permute(0, 1, 3, 2, 4).contiguous(), inputs_2d_gt[:, pad:pad + 1, :, :].permute(0, 1, 3, 2, 4).contiguous().to(out.device), weight_2d.to(out.device), pad) + 2*loss_3D_triang
            ...
            print(pose_3D_in_cam_space.permute(0, 1, 2, 4, 3).contiguous().shape)
            print(p3d_gt_abs[:, pad:pad + 1].permute(0, 1, 2, 4, 3).contiguous().shape)

            loss_copy = mpjpe(pose_3D_in_cam_space.permute(0, 1, 2, 4, 3).contiguous(), p3d_gt_abs[:, pad:pad + 1].permute(0, 1, 2, 4, 3).contiguous())
        ...
        print('epoch:{:3} time:{:.2f} lr:{:.9f} best_result_epoch:{:3} best_result:{:.3f}'.format(epoch, elapsed, lr,
                                                                                                  best_result_epoch,
                                                                                                  best_result))
